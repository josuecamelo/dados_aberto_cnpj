        ...
        with zipfile.ZipFile(arquivo) as z:
            ...
            with z.open(csvFile) as f:
                ...
                for row in df.iterrows():
                    ...
                    sql = "INSERT INTO dados_simples(cnpj_basico, opcao_simples, data_opc_simples, data_exclusao_simples, opcao_mei, data_opc_mei, data_excl_mei) " \
                        "VALUES ('" + str(row[1][0]).zfill(8) + "', '" + escapeSpecialCharacters(str(row[1][1])) + "', '" + str(row[1][2]) + "'" \
                        ", '" + str(row[1][3]) + "', '" + str(row[1][4]) + "', '" + str(row[1][5]).zfill(2) + "', '" + str(row[1][6]) + "');"
